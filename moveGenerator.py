...
def simpleMove(roll, pos):
    ...
    if roll[0] == roll[1]:  ##rolled doubles so get 4 copies
        scheme = np.array(
            [str(roll[0]), str(roll[0]), str(roll[0]), str(roll[0])]
        ).reshape((1, 4))
    else:
        scheme = np.array(
            [str(roll[0]), str(roll[1]), str(roll[1]), str(roll[0])]
        ).reshape((2, 2))
    ...
    for i in range(finalPos.shape[0]):
        if (
            finalPos[i, 0] == finalPos[i, 1]
            and finalPos[i, 0] != 101
            and finalPos[i, 0] != 0
        ):
