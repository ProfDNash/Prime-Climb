...
def takeTurn(playerNum, PlayerList, Primes, Deck, DiscardPile, Spots, printData):
    ...
    possibleMoves = moveMapper(
        roll, pos, PlayerList[playerNum].cards, PlayerList[playerNum].cursed, Spots
    )
    ...
    if Move.shape[0] > 2 and Move[2] != 100000000000:
        ...
        for j in range(len(matches)):
            if sum(int(digit) for digit in str(possibleMoves[matches[j], 2])[1:]) < sum(
                int(digit) for digit in str(bestMove[2])[1:]
            ):
    ...
    rollAgain, PlayerList, Deck, DiscardPile = drawACard(
        playerNum, pos, Move[0:2], PlayerList, Primes, Deck, DiscardPile
    )
