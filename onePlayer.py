...
def onePlayerGame(parameters, lambd=0, alpha=0.1, eps=0, learn=True):
    ##initialize the beginning of the game##
    params = parameters.copy()
    Xt = np.array([1, 0, 1]).reshape((3, 1))
    Xlist = np.array(Xt, copy=True).reshape((3, 1))
    ...
    while Xt[0, -1] != 101 and Xt[2, -1] != 101 and turn < 10000:
        ...
        if (
            Xt[0, -1] == 101
        ):  ##if player 0 wins, feed final reward instead of forward prop result
            Yt = np.array([1, 0]).reshape((2, 1))
        grads = backProp(Yt, Ylist[:, turn - 1], cacheList[turn - 1])
        gradcache[turn] = grads
        if learn == True:
            params = updateParameters(
                Yt, Ylist[:, turn - 1], turn, gradcache, params, lambd, alpha
            )
