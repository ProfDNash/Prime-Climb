...
def drawACard(playerNum, oldPos, newPos, PlayerList, Primes, Deck, DiscardPile):
    ...
    if newPrimeFlag == 2:
        newPrimeFlag = np.random.randint(
            0, 2
        )  ##now newPrimeFlag denotes the pawn chosen
        ...
        if Draw > 13:
            ...
            if Draw < 17:  ##roll Again
                ...
                for i in range(len(PlayerList)):
                    ...
                    if (
                        PlayerList[i].position[1] != 101
                    ):  # ignore pawns that have already left the board
                        currLocs.append(PlayerList[i].position[1])
                currLocs.sort()
                ##locate chosen pawn in the list
                idx = currLocs.index(newPos[newPrimeFlag])
                if (
                    idx != len(currLocs) - 1 and Draw == 19
                ):  ##move forward if possible to nearest pawn and (later) bump it back to start
                    newPos[newPrimeFlag] = currLocs[idx + 1]
                    posChangeFlag = 1
                elif (
                    idx != 0 and Draw == 20
                ):  ##move backward if possible to nearest pawn and (later) bump it back to start
                    newPos[newPrimeFlag] = currLocs[idx - 1]
                    posChangeFlag = 1
            elif Draw == 21:  ##reverse digits
                digits = str(
                    newPos[newPrimeFlag]
                )  ##there are always exactly two digits on (eligible) Primes
                if (
                    digits[0] != digits[1]
                ):  ##don't do anything if the digits are the same
                ...
                if (
                    len(PlayerList) > 1
                ):  ##If there's only one player, swapping pawns is meaningless
                    ...
                    if (
                        p1 == 1 and PlayerList[swaps[0]].position[1] == 101
                    ):  ##cannot swap with a pawn at 101
                        p1 = 0
                    ...
                    PlayerList[swaps[0]].position[p1] = PlayerList[swaps[1]].position[
                        p2
                    ]
                ...
                if len(stealable) > 0:
                    ...
                    stealcard = PlayerList[stealfrom].cards[
                        np.random.randint(0, len(PlayerList[stealfrom].cards))
                    ]
    ...
    if (
        posChangeFlag == 1
    ):  ##an action card changed the players position, so recheck for bumping
