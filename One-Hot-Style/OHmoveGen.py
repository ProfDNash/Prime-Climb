...
def OHsimpleMove(roll, pos):    
    ...
    finalPos = np.array([]).reshape((0,102))  #initialize the array of all complete moves
    for order in scheme:
        iP = pos.copy()  ##hold the initial position
        for item in order:
            posList = np.array([]).reshape((0,102)) ##initialize list of potential moves
            for i in range(iP.shape[0]):
                posList = np.append(posList, OHapplyDie(iP[i,:].reshape((1,102)), item),axis=0)
            iP = posList.copy()
